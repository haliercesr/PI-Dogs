export default function validations(user) {
    ...
    //VALIDACIONES NOMBRE
    if (!user.name) {
        errors.name = "Porfavor ingresar un nombre";
    ...
    if (!user.weightMin || !user.weightMax) {
        errors.weight = "Por favor ingresar altura Min. y Max."
    } else if (user.weightMin <= 0 || user.weightMax > 200) {
        errors.weight = "Debe ser mayor a 0 y menos que 200"
    } else if (user.weightMin >= user.weightMax){
        errors.weight = "Primer valor mayor que el segundo"
    }else { errors.weight = "" }

    //VALIDACIONES AÑOS DE VIDA
    if (!user.life_span) {
        errors.life_span = "Por favor los años de vida"
    } else if (user.life_span < 0 || user.life_span > 50) {
        errors.life_span = "Debe ser positivo y menor a 100"
    ...
    if (!user.selectedTemperaments || user.selectedTemperaments.length === 0) {
        errors.selectedTemperaments = "Debe tener al menos 1 temperamento";
    } else if (user.selectedTemperaments.length > 5) {
        errors.selectedTemperaments = "No puede tener más de 5 temperamentos";
    } else {
